...
포트폴리오 코치의 **10가지 평가 기준**은 카카오톡 오픈채팅방 **"소프트웨어 마에스트로 준비방"** 에서 **엄지척 재이지(SW마에스트로 15기 엑스퍼트)** 님이 공유해주신 포트폴리오 꿀팁을 기반으로 만들어졌습니다.

> 참고: SW마에스트로에서 **엑스퍼트**는 **멘토와 별개의 역할**입니다. 두 역할이 다르므로 정확하게 표기해주시기 바랍니다.
...
이 도구의 평가 기준은 원작자(엄지척 재이지, SW마에스트로 15기 엑스퍼트)의 통찰에 기반합니다. 코드 자체는 학습/참고 목적으로 자유롭게 이용 가능합니다. 단, 평가 기준 원본을 인용할 때는 원작자를 명시해주세요.
